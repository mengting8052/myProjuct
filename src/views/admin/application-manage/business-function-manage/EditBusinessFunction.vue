  ...
  <div>
    <Form ref="form" :model="form" :rules="rules">
      <div class="search_box">
        ...
        <!--上级功能-->
        <FormItem prop="parentIds" :label="$t('businessFunctionManage.form.label.parents')" :label-width="110"
                  style="width: 310px;">
          <Cascader :data="functionOptions" v-model="form.parentIds"  @on-change="onParentChange"
        ...
        <!--排序值-->
        <FormItem prop="sort" :label="$t('businessFunctionManage.form.label.sort')" :label-width="110"
                  v-if="form.type == 'menu'">
          <Input class="input_width" v-model="form.sort" clearable/>
          <span>({{$t("businessFunctionManage.text.range")}})</span>
        </FormItem>
        <!--菜单图标-->
        <FormItem prop="icon" :label="$t('businessFunctionManage.form.label.icon')" :label-width="110"
                  v-if="form.type == 'menu'">
          <hero-icon :name="form.icon" width="20" height="20"></hero-icon>
          <Button type="primary" @click="iconModalShow = true">{{$t("businessFunctionManage.button.selectIcon")}}
          </Button>
        </FormItem>
      </div>
      <!--关联权限-->
      ...
      <div>
        ...
        <Button type="ghost" @click="$router.push({name: 'business-function-manage'})">
          {{$t("common.form.btn.cancel")}}
        </Button>
      </div>
...
export default {
  ...
  methods: {
    ...
    loadData(id) {
      ...
      this.loadFunctionOptions().then(res => {
        getBusinessFunctionDetail(id).then(res => {
          this.form = res.data;
          this.tempParent.parentId = this.form.parentId;
          let temp = Number(this.form.sid);
          this.form.sid = temp;
          //遍历上级功能赋值
          this.traverseParentsFunction(this.form.parentId);
          this.traverseAuthorityData();
        });
      });
    },
    /**
     ...
     * 根据接口数据,赋值  关联权限
     */
    traverseAuthorityData() {
      let tempArray = {};
      let _this = this;
      if (this.form.sysList.length > 0) {
        // 获取关联权限的模块
        this.subsystemTable.ids = [];
        this.form.sysList.forEach(item => {
          this.subsystemTable.ids.push(item.sid);
          if (!tempArray[`${item.sid}`]) {
            tempArray[`${item.sid}`] = [];
          }
          tempArray[`${item.sid}`].push({
            mid: item.mid,
            sysFunctionIds: item.sysFunctionIds
          });
        });

        new Promise(resolve => {
          fetchSubSystemModule(this.subsystemTable.ids).then(res => {
            this.collapseData = res.data;
            resolve();
          });
        }).then(res => {
          // 根据模块对相应的checkbox打√
          this.collapseData.forEach((k, i) => {
            k.moduleList.forEach((v, y) => {
              if (tempArray[k.id]) {
                tempArray[k.id].forEach((j, x) => {
                  if (v.id == j.mid) {
                    _this.$set(
                      _this.collapseData[i].moduleList[y],
                      "checkGroup",
                      j.sysFunctionIds
                    );
                  }
    ...
    /**
     * 上级功能 - 根据接口获取到的 parentId,遍历数据获取到对应的父级
     */
    traverseParentsFunction(parentId) {
      let _this = this;
      this.form.parentIds = [];
      this.functionOptions.forEach(k => {
        k.children.forEach(v => {
          if (v.value == parentId) {
            _this.form.parentIds.push(k.value);
            _this.form.parentIds.push(v.value);
          }
          v.children.forEach(y => {
            if (y.value == parentId) {
              _this.form.parentIds.push(k.value);
              _this.form.parentIds.push(v.value);
              _this.form.parentIds.push(y.value);
            }
          });
        });
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (!this.tempParent.parentId) {
            ...
            if (
              this.tempParent.parentId != "" &&
              this.tempParent.parentId.indexOf(this.$route.query.id) > -1
            ) {
              this.$Message.error(
                this.$t("businessFunctionManage.rules.relevanceSelf")
              );
              return;
            } else {
          ...
          if (this.parentSelect.length && this.parentSelect.length === 1) {
            this.$Message.error(
