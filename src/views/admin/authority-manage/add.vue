...
<script>
import {add} from "@/api/admin/authorityList"
      ...
      return{
        form:{
            ...
            rules:{
                ...
                name:[{required:true,message:this.$t("authorityManage.form.rules.name"),trigger: 'blur'}],
                uri:[{required:true,message:this.$t("authorityManage.form.rules.uri"),trigger: 'blur'}],
                httpMethod:[{required:true,message:this.$t("authorityManage.form.rules.httpMethod"),trigger: 'blur'}],
                mid:[{required:true,type:"number",message:this.$t("authorityManage.form.rules.module"),trigger: 'blur'}]
            }
        },
    ...
    methods:{
       //模块chang事件找到sid
        changeSubSystem(val){
            ...
            const found=this.moduleList.find(item => item.id == val);
            this.form.data.sid = found.sid
        },
